...
def push_messages_workout_reminder():  # сообщение о записи на тренировку на завтра
    ...
    set_push_messages = set(i.user_id for i in df_record.itertuples() if i.date == tomorrow)
    for i in set_push_messages:
        bot.send_message(chat_id=i, text=mess, parse_mode='html')
    for i in df_record.itertuples():
        if i.date == tomorrow:
            for y in df_tr.itertuples():
                if i.id_workout == y.id_workout:
                    mess = f'Тренировка - {y.title}\nВремя - {y.time}'
                    bot.send_message(chat_id=i.user_id, text=mess, parse_mode='html')
...
def get_user_text(message):
    ...
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=1)
    if message.chat.id in _user_name_to_manager:
        ...
        for i in df_record.itertuples():
            ...
            for x in df_tr.itertuples():
                if x.id_workout == i.id_workout:
                    training_time = (datetime.datetime.strptime(i.date, '%d.%m.%Y') + timedelta(hours=x.time.hour))
            if training_time >= today:
        ...
        for i in df_tr.itertuples():
            if i.title == _trainings_to_records[user_id_to_dict]['training'] and i.day_of_the_week == \
                    _trainings_to_records[user_id_to_dict]['day'] and i.time == _trainings_to_records[user_id_to_dict][
                'time']:
                _trainings_to_records[user_id_to_dict]['id_training'] == i.id_workout
...
def run_push_message():
    ...
    schedule.every().day.at("16:12").do(push_messages_workout_reminder)
    while True:
