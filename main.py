...
class Session:
    ...
    def __init__(self):
        ...
        with open(config.ID_MANAGER_FILE_PATH, 'r') as manager_id_file:
            self.manager_id = int(manager_id_file.read())

        self.bot = telebot.TeleBot(self.TOKEN)
        self.database = peewee.SqliteDatabase(config.DB_FILE_PATH)
        self.client_choice = defaultdict(list)
        self.club_card_to_save = defaultdict(dict)
        self.users_buy_card = {}
        self.names_of_trainings_set = set()
        self.trainings_by_date = {}
        self.trainings_to_records = defaultdict(dict)
        self.user_name_to_manager = {}
        self.name_of_coaches_set = set()
...
def check_phone_number_for_manager(user_id, message, markup):
    ...
    if phone_number:
        ...
        session.bot.send_message(chat_id=session.manager_id, text=mess_to_manager, parse_mode='html')
...
def send_coaches_names(user_id, message, coaches, markup):
    ...
    for i in coaches.itertuples():
        ...
        markup.add(coach)
        session.name_of_coaches_set.add(i.coach_name)
    markup.add(restart)
    session.bot.send_message(user_id, messages.MAIN_MENU_MESSAGE[message], reply_markup=markup)
    session.bot.send_message(user_id, messages.GO_TO_MAIN_MENU_MESSAGE, parse_mode='html')
...
def request_for_personal_correspondence(username, user_id, message, markup):
    if not username:
        ...
        session.bot.send_message(user_id, messages.MAIN_MENU_MESSAGE[message], reply_markup=markup)
        session.bot.send_message(user_id, messages.GO_TO_MAIN_MENU_MESSAGE, parse_mode='html')
        session.bot.send_message(chat_id=session.manager_id, text=mess, parse_mode='html')
                    ...
                    if i.training_id == y.training_id:
                        for z in training_types.itertuples():
                            if z.training_type_id == y.training_type_id:
                                ...
                                if mess_0:
                                    session.bot.send_message(chat_id=user_id, text=mess_0, parse_mode='html')
                                    mess_0 = None
                                session.bot.send_message(user_id, mess)
                                user_record = True
                                break
                        break
    if not user_record:
        session.bot.send_message(user_id, messages.YOU_HAVE_NO_ENTRY_MESSAGE)
    session.bot.send_message(user_id, messages.GO_TO_MAIN_MENU_MESSAGE, reply_markup=markup)


def send_workout_names(user_id, markup, training_types):
    mess = messages.CHOICE_TRAININGS_MESSAGE
    session.names_of_trainings_set = sorted(set((i.title) for i in training_types.itertuples()))
    for y in session.names_of_trainings_set:
...
def send_training_day_of_the_week_and_time_(user_id, user_id_to_dict, message, markup, trainings, training_types):
    for i in training_types.itertuples():
        if i.title == message:
            session.trainings_to_records[user_id_to_dict]['training_type_id'] = i.training_type_id
    session.trainings_to_records[user_id_to_dict]['training'] = message
    for i in trainings.itertuples():
        if i.training_type_id == session.trainings_to_records[user_id_to_dict]['training_type_id']:
...
def send_available_training_dates(user_id, user_id_to_dict, record, trainings, markup, day_for_training,
    ...
    session.trainings_to_records[user_id_to_dict]['day'] = day_for_training.group()
    session.trainings_to_records[user_id_to_dict]['time'] = time_for_training.group()
    for i in trainings.itertuples():
        if i.training_type_id == session.trainings_to_records[user_id_to_dict]['training_type_id'] and \
                i.day_of_the_week == session.trainings_to_records[user_id_to_dict]['day'] and \
                i.time.strftime('%H:%M') == session.trainings_to_records[user_id_to_dict]['time']:
            session.trainings_to_records[user_id_to_dict]['training_id'] = i.training_id
    ...
    for i in available_dates:
            ...
            if y.date == i.strftime("%d.%m.%Y"):
                for x in trainings.itertuples():
                    if x.training_id == y.training_id and \
                            x.day_of_the_week == session.trainings_to_records[user_id_to_dict]['day'] and \
                            x.time.strftime('%H:%M') == session.trainings_to_records[user_id_to_dict]['time']:
                        for z in training_types.itertuples():
                            if x.training_type_id == z.training_type_id and \
                                    z.title == session.trainings_to_records[user_id_to_dict]['training']:
                                signed_up_people += 1
                                break
        if (max_people - signed_up_people) <= 0:
            ...
            continue
        for y in trainings.itertuples():
            if y.training_id == session.trainings_to_records[user_id_to_dict]['training_id']:
                for x in coaches.itertuples():
                    if x.id_coach == y.id_coach:
                        session.trainings_to_records[user_id_to_dict]['coach'] = x.coach_name
...
def checking_re_entry_for_training_and_recording(user_id, user_id_to_dict, message,
    ...
    if record:
        ...
        data_record = ({'user_id': user_id,
                        'training_id': session.trainings_to_records[user_id_to_dict]['training_id'],  # Падает Бот!
                        'date': session.trainings_to_records[user_id_to_dict]["date"]
                        })
        ...
        mess = messages.get_training_session_message(training=session.trainings_to_records[user_id_to_dict]["training"],
                                                     date=session.trainings_to_records[user_id_to_dict]["date"],
                                                     time=session.trainings_to_records[user_id_to_dict]["time"],
                                                     coach=session.trainings_to_records[user_id_to_dict]["coach"])
        markup.add(restart)
...
def send_list_club_cards(user_id, _user_club_cards, club_cards, user_id_to_dict, markup):
    ...
    for i in club_cards.itertuples():
        ...
        session.client_choice[user_id_to_dict].append(button)
    session.bot.send_message(user_id, mess, reply_markup=markup)
    restart = '/start'
    markup.add(restart)
    session.bot.send_message(user_id, messages.GO_TO_MAIN_MENU_MESSAGE, reply_markup=markup)
...
def get_confirmation_of_purchase(user_id, user_name, markup, message, user_club_cards):
    session.users_buy_card[user_id] = message
    restart = '/start'
...
def registration_club_card_in_db(user_id, markup, club_card, user_name):
    ...
    for i in club_card.itertuples():
        if i.title in session.users_buy_card[user_id]:  # создание данных для записи в базу
            date = datetime.datetime.now()
            date_end = date + (timedelta(i.validity))
            session.club_card_to_save[user_id_to_dict]['user_id'] = user_id
            session.club_card_to_save[user_id_to_dict]['name'] = user_name
            session.club_card_to_save[user_id_to_dict]['title'] = i.title
            session.club_card_to_save[user_id_to_dict]['validity'] = i.validity
            session.club_card_to_save[user_id_to_dict]['price'] = i.price
            session.club_card_to_save[user_id_to_dict]['date_of_buy'] = date.strftime("%d.%m.%Y")
            session.club_card_to_save[user_id_to_dict]['date_of_the_end'] = date_end.strftime("%d.%m.%Y")
    mess = messages.get_a_purchased_club_card(name=session.club_card_to_save[user_id_to_dict]['name'],
                                              title=session.club_card_to_save[user_id_to_dict]["title"],
                                              price=session.club_card_to_save[user_id_to_dict]["price"],
                                              date_of_buy=session.club_card_to_save[user_id_to_dict]["date_of_buy"],
                                              date_of_the_end=session.club_card_to_save[user_id_to_dict][
                                                  "date_of_the_end"]
                                              )
    session.bot.send_sticker(user_id, sticker)
    session.bot.send_message(user_id, mess, reply_markup=markup)
    client_in_db = ClubCards.insert_many(
        session.club_card_to_save[user_id_to_dict]).execute()  # запись купленного абонемента в базу
    del session.users_buy_card[user_id]
    del session.club_card_to_save[user_id_to_dict]
...
def no_valid_answer_options(user_id, markup):
    restart = '/start'
    markup.add(restart)
    session.bot.send_message(user_id, messages.CHOICE_ERROR_MESSAGE, reply_markup=markup)
...
def start(message):
    ...
    for i in main_menu:
        markup.add(i)
    session.bot.send_sticker(message.chat.id, sticker)
    session.bot.send_message(message.chat.id, mess, reply_markup=markup)


@session.bot.message_handler(content_types=['text'])  # обработка текстовых запросов от пользователя
def get_user_text(message):
    ...
    elif message.text in session.name_of_coaches_set:  # информация о тренерах
    ...
    elif message.text in session.names_of_trainings_set:  # Меню с днем недели + время тренировки
    ...
    elif message.text in session.client_choice[user_id_to_dict]:  # подтверждение о покупке абонемента
...
def run_bot():
    session.bot.polling(none_stop=True)
